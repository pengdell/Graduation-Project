...
class Model:
    def __init__(self, args):
        ...
        self.cv = args.cv
        self.dataSet = DataSet(self.dataName, self.cv)
        ...
        self.ratedItems = self.dataSet.ratedItems
        self.topKNeg = self.dataSet.getTopKNeg(self.data, 100)
    ...
    ##get teacher's topk predictions
    def getTopK(self, sess, K):
        User = self.topKNeg[0]
        Item = self.topKNeg[1]
        ...
        for i in range(len(User)):
            ...
            knownItems = self.ratedItems[currentUser]
            
            feed_dict = self.create_feed_dict(User[i], Item[i])
            ...
            fout = open('./_Knowledge/' + self.dataName + '/predictions.txt', 'a')
            
            k = 0

            for t in range(self.shape[1]):
                if full_ranklist[t] not in knownItems:
                    if k < K:
                        pred = str(full_ranklist[t]) + '\t'
                        fout.write(pred)
                        k += 1
                    else:
                        fout.write('\n')
